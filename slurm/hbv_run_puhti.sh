...
#SBATCH --output=/scratch/project_XXXXXXX/hbv/logs/%j_%a.out
#SBATCH --error=/scratch/project_XXXXXXX/hbv/logs/%j_%a.err
# Array size set by API: sbatch --array=0-N hbv_run_puhti.sh

module load apptainer

SIF=/scratch/project_XXXXXXX/hbv/hbv-compute.sif
TOTAL=${SLURM_ARRAY_TASK_COUNT}
TASK=${SLURM_ARRAY_TASK_ID}

mkdir -p "${HBV_OUTPUT_DIR}"
mkdir -p /scratch/project_XXXXXXX/hbv/logs
